...
namespace NURBS
{
    ...
    public class Surface
    {
        ...
        {
            //Validate degree
            if (degreeU >= controlPoints.GetLength(0))
                Debug.LogError("NURBS degree U must be < number of control points in U");
            if (degreeV >= controlPoints.GetLength(1))
                Debug.LogError("NURBS degree V must be < number of control points in V");
        ...
        public Surface(ControlPoint[,] controlPoints, int degreeU, int degreeV, float[] knotsU, float[] knotsV)
        {
            ...
            //Valide knots
            if (knotsU.Length != controlPoints.GetLength(0) + degreeU + 1)
                Debug.LogError("NURBS knots U length must be = number of control points in U + degreeU + 1");
            if (knotsV.Length != controlPoints.GetLength(1) + degreeV + 1)
                Debug.LogError("NURBS knots V length must be = number of control points in V + degreeV + 1");
        ...
        {
            if (u < 0 || u > 1)
                Debug.LogError("U coordinate must be [0, 1]");
            if (v < 0 || v > 1)
                Debug.LogError("V coordinate must be [0, 1]");
